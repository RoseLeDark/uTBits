#ifndef _UT_ITERRATOR_H_
#define _UT_ITERRATOR_H_
...
namespace utb {
    ...
    namespace internal {
        ...
        distance(TIter first, TIter last, utb::random_access_iterator_tag) {
            return last - first;
        }

        template<typename TIter>
        constexpr typename iterator_traits<TIter>::difference_type
        distance(TIter first, TIter last, utb::input_iterator_tag) {
        ...
        constexpr void advance(TIter& iter, TDistance d, utb::random_access_iterator_tag) {
            iter += d;
        }
        template<typename TIter, typename TDistance = typename iterator_traits<TIter>::difference_type>
        constexpr void advance(TIter& iter, TDistance d, utb::bidirectional_iterator_tag) {
            if (d >= 0) {
                while (d--) ++iter;
            } else {
                while (d++)  --iter;
        ...
        template<typename TIter, typename TDistance = typename iterator_traits<TIter>::difference_type>
        constexpr void advance(TIter& iter, TDistance d, utb::input_iterator_tag) {
            assert(d >= 0);
            while (d--)  ++iter;
        }
    }
	...
	public:
		...
		constexpr difference_type	operator -  (const self_type& i) const {
			return utb::distance (i.m_itter, m_itter); }
	protected:
		iterator m_itter;
	};
